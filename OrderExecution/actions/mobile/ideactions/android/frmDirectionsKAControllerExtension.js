...
kony.sdk.mvvm.frmDirectionsKAControllerExtension = Class(kony.sdk.mvvm.CustomFormControllerExtension, {
    $statics: {
        MAP_CURRENT_LOCATION_IMAGE: "current_location.png",
        MAP_CURRENT_LOCATION_DESC: "Current Location",
        MAP_DESTINATION_LOCATION_IMAGE: "map_pin_today.png"
    },
    ...
    bindData: function(dataMap) {
        try {
            kony.print("success");
            var flag = this.getCurrentLocation();
        } catch (error) {
    ...
    getCurrentLocation: function() {
        try {
            ...
            var notifiExecuControllerExtension = appContext.getFormController("frmOrderExecutionKA").getControllerExtensionObject();
            var destLat = notifiExecuControllerExtension.getFormModelInfo("destinationLat");
            var destLon = notifiExecuControllerExtension.getFormModelInfo("destinationLon");
            var mapLocationOrderDirection = [];
            var index;
            formModel.setViewAttributeByProperty("mapDirectionKA", "zoomLevel", kony.servicesapp.mapMyOrderListKAZoomValueKA);
            var gpsSuccess = function(location) {
                    ...
                    index = 0;
                    var currentLocation = scopeObj.addMapLocation(index, currentLocations.lat, currentLocations.lon, kony.sdk.mvvm.frmDirectionsKAControllerExtension.MAP_CURRENT_LOCATION_IMAGE);
                    mapLocationOrderDirection.push(currentLocation);
                    if (destLat && destLon) {
                        index = 1;
                        var destLocation = scopeObj.addMapLocation(index, destLat, destLon, kony.sdk.mvvm.frmDirectionsKAControllerExtension.MAP_DESTINATION_LOCATION_IMAGE);
                        mapLocationOrderDirection.push(destLocation);
                    }
    ...
    addMapLocation: function(index, lat, lon, img) {
        var location = {
            ...
            lat: lat,
            lon: lon,
            name: kony.sdk.mvvm.frmDirectionsKAControllerExtension.MAP_CURRENT_LOCATION_DESC,
            desc: kony.sdk.mvvm.frmDirectionsKAControllerExtension.MAP_CURRENT_LOCATION_DESC,
            image: img,
            meta: {
                color: "blue",
                label: "A"
