function ConfigProviderMFAPPOnline(backendUrl){
	...
	function getConfigurationFromBundle(bundleId, configurationObject, successCallback){
		try{
			...
			httpclient.onReadyStateChange = function(){
					...
					for(var i=0;i<configurations.length;i++){
						if(configurations[i].type!=undefined){
						  while(configurations[i].value.search("'")!= -1){
							...
							configDataObject[configurations[i].key] = configurations[i].value;
						}
					}
				...
				if(typeof successCallback == 'function')successCallback(configurationObject);
			}
			httpclient.open(constants.HTTP_METHOD_GET, encodeURI(backendUrl+"/ConfigurationSDO/objects/Configurations?$filter=configurationBundle_id eq "+bundleId));
			httpclient.send();
		}catch(e){
			alert(e);
		}
	...
	this.getConfigurations = function(contextObject, successCB){
		var url = backendUrl+"/ConfigurationSDO/objects/ConfigurationMaster?$filter=app_id eq ORDER_EXECUTION";
		getConfigurationMasters(url, successCB);
	}
}
...
var AppConfigurationController = (function(){
	...
	function createInstance(backendUrl){
		//var configFactory = ConfigurationFactory.getInstance();
        var bURL =  backendUrl;
		var configProvider = new ConfigProviderMFAPPOnline(bURL);//configFactory.getConfigurationProvider();
		var configurationFileLocation = kony.io.FileSystem.getDataDirectoryPath() + "/FSConfigurations.txt";
		...
		this.getConfigurations = function(context, successCB, isRefresh){
			if(isRefresh){
				var contextObject = {};
				contextObject.type = context.type;
				contextObject.roleId = context.userID;
				...
				var success = function(configurationObject){
					...
					//if(!(JSON.stringify(configurationObject)=="{}")){
						if(myFile.exists())myFile.write(configurationObject.stringify());
						else{
