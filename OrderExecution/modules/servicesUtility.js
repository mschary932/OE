var utilities = Class({
    ...
    constructor: function() {
        ...
        this.statusImage = {
      	...
      	this.typeImage = {
            "Measurement": "measurement_icon_small_brown.png",
            "MEAS": "measurement_icon_small_brown.png",
            "ORD": "",
          	" ": ""
            };
   ...
   getPrioritySkinKA: function(priorityValue) {
    ...
    getStatusImageKA: function(statusValue) { //getting status image
  	...
  	getMeasurementIconKA: function(typeValue) { //getting measure icon image
        try {
            return (this.typeImage[typeValue] == null ? "" : this.typeImage[typeValue]);
        } catch (err) {
            kony.sdk.mvvm.log.error("error in Blogic get gettypeImage : " + err);
        }
    },
    ...
    getNearMeTextKA: function(nearMeValue){    
     	try {
			...
			if (kony.servicesapp.constants.getServiceConstantsObj().getUOM("UOM") == "KMS") {
				extension = this.geti18nValueKA("i18n.common.Map.lblKilometerKA.valueKA");
			}
			else {
				extension = this.geti18nValueKA("i18n.common.uom.distance");
			}
  ...
  deleteRecord:function(rowData){
    	...
    	formModel.setViewAttributeByProperty("lblIdKA", "text", rowData.Category_id);
        formModel.setViewAttributeByProperty("lblTypeKA", "text", rowData.Description);
    	dismissPopUp("deleteRecord",3, this.updateRecord);
  },
  updateRecord:function(){  
		...
		var formmodel = controller.getFormModel();
		var rowData = formmodel.getViewAttributeByProperty("SegTimeExpenseKA", "data")[section][1][row];
       if(rowData.Task_id){
			var modelObjToBeUpdated="TaskTimeExpense"
		}
		else{
			var modelObjToBeUpdated="WorkOrderTimeExpense"
		}
        var navigationObject = new kony.sdk.mvvm.NavigationObject();
        var TimeExpenseModelObject = INSTANCE.getModel(modelObjToBeUpdated, kony.servicesapp.OE_OBJECT_SERVICE_NAME, kony.servicesapp.APP_OPTIONS);
        var dataObject = new kony.sdk.dto.DataObject(modelObjToBeUpdated);
        var updateField = {};
        updateField.id = rowData.id;
        ...
        function OnError(err) {      
			...
			formmodel.setViewAttributeByProperty("flxDeleteKA","isVisible",false);
            formmodel.performActionOnView("flxContentKA", "setEnabled",[true]);	         
          	frmTimeAndExpenseKA.forceLayout();
          	currentFormControllerExtension.fetchData();
        }  
		}
		catch(err)
  ...
  deleteQuantity:function(rowData){
		...
		//var selRecord = formModel.getViewAttributeByProperty("segTaskExecutionKA", "selectedItems")[0];
        formModel.setViewAttributeByProperty("lblHeaderDeleteddKA","text","Deleted");
        ...
        formModel.setViewAttributeByProperty("imgStatusKA","src","bf_order_resource_delete.png");
        formModel.setViewAttributeByProperty("btnRefreshKA", "isVisible",true);
        //formModel.setViewAttributeByProperty("flxDeleteKA", "isVisible",true);			
		formModel.performActionOnView("flexDetailsKA","setEnabled",[false]);
		formModel.performActionOnView("btnBackKA","setEnabled",[false]);
		formModel.performActionOnView("btnBackKA","setEnabled",[false]);
  ...
  showStockTransferPopup: function(isPurchaseRequest, rowData){
        var INSTANCE = kony.sdk.mvvm.KonyApplicationContext.getAppInstance();
        var controller = INSTANCE.getFormController(kony.application.getCurrentForm().id);
        var formModel = controller.getFormModel();
        ...
        if(isPurchaseRequest){
            // var displayText="Your purchase request "+"has been raised";
            var displayText=utilitiesObj.geti18nValueKA("i18n.purchaseRequest.confirmation");
            formModel.setViewAttributeByProperty("lblOrderMessageKA","text", displayText);
        }else{
            // var displayText="The stock transfer request "+"has been raised with "+"Stock location name";
            // var displayText="The stock transfer request "+"has been raised";
            var displayText=utilitiesObj.geti18nValueKA("i18n.transferRequest.confirmation");
            formModel.setViewAttributeByProperty("lblOrderMessageKA","text", displayText);
        }
        var confirmationText=utilitiesObj.geti18nValueKA("i18n.common.ConfirmationKA");
        formModel.setViewAttributeByProperty("lblHeaderDeleteddKA","text",confirmationText);
        formModel.setViewAttributeByProperty("lblOrderNumberKA","text",rowData.MaterialName);
        formModel.setViewAttributeByProperty("lblDeleteResourceNumberKA","text",rowData.RequestedQuantityNumber);
        formModel.setViewAttributeByProperty("lblResourceQuantityKA","text"," ");
        formModel.setViewAttributeByProperty("imgStatusKA","src","bf_resource_ex_stock_info.png");
        formModel.setViewAttributeByProperty("btnRefreshKA", "isVisible",false);
        //formModel.setViewAttributeByProperty("flxDeleteKA", "isVisible",true);            
        formModel.performActionOnView("flexDetailsKA","setEnabled",[false]);
        formModel.performActionOnView("btnBackKA","setEnabled",[false]);
        if(kony.application.getCurrentForm()["btnOptionsKA"]){formModel.performActionOnView("btnOptionsKA","setEnabled",[false]);}
...
function navigationOnGesture(myWidget, gestureInfo){  
	try{
		...
		}else if (myWidget.id == "flxWorkConfirmationKA"){
			frmWorkConfirmationKA.show();
...
function showHideHamburgerMenuKA1(formName,hamburgerMenuName,showView,flexName,scb){
    try{
        if(showView){           
            var tempFlex = hamburgerMenuName[flexName];
            var menuFlexRef = hamburgerMenuName[flexName];
