...
Calendar.DEFAULT_CONFIG = {
	...
	ENABLED_SKIN_DATES_ARRAY : {key:"enableskindatesarray", value:null},
...
Calendar.prototype = {
    ...
    */
    ...
    setupConfig : function() {
        ...
        */
	    ...
	    /** 
        * True if the Calendar should allow selection of selected dates before end date. False by default.
        * @config ENABLED_SKIN_DATES_ARRAY
        * @type Boolean
        * @default false
        */
        cfg.addProperty(DEF_CFG.ENABLED_SKIN_DATES_ARRAY.key, { value:DEF_CFG.ENABLED_SKIN_DATES_ARRAY.value, handler:this.configEnableSkindDatesArray } );
	...
	* 
    * The default handler for the "enabled end date" property
    * @method configEnableSkindDatesArray
    */
    configEnableSkindDatesArray : function(type, args, obj) {
        
			var val = args[0];
		if(val != null && Lang.isString(val))
		{
			//val = this._parseDates(val);
			var enabledskindatesarr = new Array();
			enabledskindatesarr = val.split(",");
			
			for (var i=0;i<enabledskindatesarr.length;++i) {
				var aDate =enabledskindatesarr[i];
				
				if (Lang.isString(aDate)) {
				
				var arr = this._parseDate(aDate);
				}
				enabledskindatesarr[i] = DateMath.getDate(arr[0],(arr[1]-1),arr[2]);
			}
			this.cfg.setProperty(DEF_CFG.ENABLED_SKIN_DATES_ARRAY.key, enabledskindatesarr);
		}
    },
    ...
    */
    renderBody : function(workingDate, html) {
        ...
        var weekNum,
			...
			enableskindatesarray = cfg.getProperty(DEF_CFG.ENABLED_SKIN_DATES_ARRAY.key),
        ...
        for (var r = 0; r < 6; r++) {
			...
			if(enableskindatesarray != null)
            {
			       var flag = false;
                    for (var s=0;s<enableskindatesarray.length;++s) {
                            var aDate =enableskindatesarray[s];
							...
							if(!flag) {
							cellRenderers[cellRenderers.length] = cal.renderCellStyleSetDateSkin;
									}
			}
			...
			if(calendarskindatesarray != null && calendarskin !=null)
            {
                    ...
                    if ((mindate && (workingDate.getTime() < mindate.getTime())) || (maxdate && (workingDate.getTime() > maxdate.getTime()))) {
						if(setcalskin != null && calendarskin != null && setcalskin)
						{
    ...
    */
    ...
    /**
    * Applies the default style used for rendering selected dates to the current calendar cell
    * @method renderCellStyleSetDateSkin
    * @param {Date}     workingDate  The current working Date object being used to generate the calendar
    * @param {HTMLTableCellElement} cell   The current working cell in the calendar
    * @return {String} YAHOO.widget.Calendar.STOP_RENDER if rendering should stop with this style, null or nothing if rendering
    *   should not be terminated
    */
    renderCellStyleSetDateSkin : function(workingDate, cell) {
	   Dom.addClass(cell, "kselected");
    },
    /**
    ...
    */
    styleEnabledSkins : function(workingDate, cell) {
		...
		YAHOO.util.Dom.addClass(cell.childNodes[0], this.Style.CSS_CALENDAR_SKIN);
    },
