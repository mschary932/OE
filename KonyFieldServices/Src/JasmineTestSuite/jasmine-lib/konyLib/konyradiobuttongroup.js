...
$KW.RadioButtonGroup = {
    ...
    updateView: function(widgetModel, propertyName, propertyValue, oldPropertyValue){
        ...
        switch (propertyName) {        
            ...
            case "selectedkey":             
				...
				//widgetModel.selectedkey = key ? key : null;
    ...
    generateList: function(radiobuttonModel, data, context){
       ...
       var htmlString = "";
		if (data.length > IndexJL) {
			...
			radiobuttonModel.selectedkey = key ? key : null; //No Key is selected
			...
			for (var i = IndexJL; i < data.length; i++) {
				if (data[i][IndexJL] != null && data[i][1+IndexJL] != null) {
					...
					if(radiobuttonModel.viewtype == "customview" && !kony.appinit.isIE8)
					{
						...
						var name = (context && context.container ? (radiobuttonModel.id + "_" + context.container.counter) : radiobuttonModel.id); //To make a new radio group for each row in segment
    ...
    eventHandler: function(eventObject, target){
        ...
        if (radiobuttonModel) {
			...
			if(radiobuttonModel.viewtype == "customview" && !kony.appinit.isIE8){
				$KW.RadioButtonGroup.changeImage($KU.getNextSibling(target),radiobuttonModel, target.value);
			} 
			...
			else
			{
                ...
                radioref && radioref.call(radiobuttonModel,radiobuttonModel);
			}
