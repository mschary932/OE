...
$KW.IGallery =
{
    ...
    updateView: function(widgetModel, propertyName, propertyValue, oldPropertyValue){
        ...
        switch (propertyName) {  
			...
			case "noofrowsperpage":
				if(element){
					element.parentNode.innerHTML = this.render(widgetModel, {tabpaneID: element.getAttribute("ktabpaneid")});;
	...
	generateImages: function(galleryModel, masterdata, index, key){
        ...
        for (var i = IndexJL; i < (masterdata.length); i++) {
            ...
            if (src) {
                ...
                // span : Span should have min height and with to comepensate background-image
                ...
                //if (isWaitAllowed) // As loading of the images is dictated by the scalemode to avoid flickering after the image is loaded once. Need to reviewed if any issue exists on the loading indicator
	 ...
	 /**     
     * $KW.IGallery.setData(frm12.gall1 ,[{iurl="Color1.png"},{iurl="RN_
	...
	/*$KW.IGallery.setDataAt(frm12.gall1 ,{iurl="RN_icon4.png"},3)*/	
	setDataAt: function(galleryModel, dataObj, index){ 
	 ...
	 modifyContent: function(galleryModel, dataArray, action, index, key){ // index starts at 1
	 
		if (galleryModel) {
			$KW.Utils.updateContent(galleryModel, "masterdata", dataArray, action, index);
			galleryModel.canUpdateUI = false;
			galleryModel.data = IndexJL ? [null, galleryModel.masterdata, key] : [galleryModel.masterdata, key]; //Updating data here as user can read without setting it.
