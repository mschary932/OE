...
$KW.DataGrid = {
			...
			case "columnheadersconfig" :
				if(kony.appinit.isIE7 || kony.appinit.isIE8){
					if(propertyValue){
						...
						widgetModel.columnheadersconfig = hConfig;
					}					
				}
                if(element){
				    var htmlstring = $KW.DataGrid.render(widgetModel, widgetModel.context);
	...
	create : function(gridModel, context) {
		...
		var computedSkin = $KW.skins.getWidgetSkinList(gridModel, gridModel.context, rowdata);
	...
	headerBuilder : function(columnModel, gridModel) {
		...
		var thTmpl = "<th id='{id}' style='" + padding + border + "' width='{width}%' class='{class}' type='{type}' colno='{colno}' colindex='{colindex}'>";
		var temp = null;
		var headerStr = "";		
		var headerHTML = [];
		var colInfo = "";
	...
	/**rowdata :[{col1="Checking",col2="490",col3="$400"}, 
	 *			 {col1="Checking",col2="495",col3="$2000",metainfo={skin=rowskin1, col2_skin=colskin1}}]	 
	 */
	createBody: function(gridModel, rowdata) {
		...
		if (rowdata.length > IndexJL) {
			var border = "word-wrap:break-word;border:" + gridModel.cellBorder;
			var textTmpl = "<td id='{id}' width='{width}%' style='" + padding + border + "' class='{class}' colindex='{colindex}' >{text}</td>";
			var imgTmpl = "<td id='{id}' width='{width}%' style='"+ padding + border + "' class='{class}' colindex='{colindex}'><img src={src} class='{class}' onload='$KW.DataGrid.imgLoadHandler(arguments[0],this)'></td>";
			var temp = null;
			var bodyrowhtml = [];
			var noofcolumns = 0;
			var id, colInfo = "";
            var colspan = 1;
			gridModel.counter = 1; //Temporary fix to render radio widgets in datagrid. Since we can have several radio groups in the same row.. fix it later
			...
			for (var i = IndexJL; i < rowdata.length; i++) {
				...
				for (var j = 0; j < noofcolumns; j++) {
					...
					if (type == "text") {
						...
						//supprot ticket: 12613: data is not rendering properly in IE8, if the text contains $_.
                        ...
                        temp = "<td id='{id}' width='{width}%' style='" + padding + border + "' class='{class}' colindex='{colindex}' colspan='" + colspan + "'>";
                        temp = temp.replace(/\{class\}/g, (skin + " " + align + " " + padding));
    ...
    eventHandler: function(event, target){
        ...
        (index > 0) && $KW.DataGrid.setFocusedIndex(gridModel, target, cell, index);
        $KW.DataGrid.rowClickHandler(gridModel, index, cell);
    },
	...
	/**     
     * $KW.DataGrid.setData("frm1.datagrid1" ,[{
     *           "two": "Newyork SetData",
     *           "one": "Hyderabad SetData"},
     *           {"two": "Washington SetData",
     *           "one": "Delhi SetData" }])     
     */	
	...
	/*$KW.DataGrid.setDataAt("frm1.datagrid1",4,{
     *       "two": "Newyork SetDataAt",
     *       "one": "Hyderabad SetDataAt"})
     */	
	...
	/*$KW.DataGrid.setCellData("frm1.datagrid1",1 ,"col1", "Savings")*/
	...
	setTBodyInnerHTML: function(tBody, html, action)
	{
		...
		{
			// The innerHTML property of a table is read-only. http://support.microsoft.com/kb/239832.
			var temp = this.createTemp();			
			if(action == "setdata"){
				temp.innerHTML = "<table>" + html + "</table>";
				while(tBody.children.length > 0){ //Remove existing head and body
				...
				if(tBody.tagName == 'TABLE') //we get table when there is no dataset initially
	...
	modifyContent: function(gridModel, dataArray, action, index, columnID){ // index starts at 1
			...
			if (holder) {											
				switch (action) {
					...
					case "addat": 
						...
						var dataArray = IndexJL ? [null, dataArray] : [dataArray];	
						wrapper.innerHTML = "<table><tr index=" + rIndex + $KW.Utils.getBaseHtml(gridModel, {tabpaneID : gridObj.getAttribute("ktabpaneid")}) + "></tr></table>";
						var innerHTML = this.createBody(gridModel, dataArray)[0].join("");
				...
				$KU.toggleVisibilty(gridObj, gridModel.data, gridModel);	
	...
	adjustNodeIndex: function(node) { //row index starts with 1 in DOM for both lua and js. Oth index refers to header row.
