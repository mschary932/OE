...
$KW.Appmenu =
    ...
    render: function(widgetID, widgetGenerationContext){
        ...
        var appmenuWidgetModel = $KG["__appmenu"];//kony.model.getWidgetModel("__appmenu");
		...
		var focusIndex = $KW.Appmenu.focusIndex;
		var appmenuSkin;
		for(var i=IndexJL, cnt=IndexJL-1; i < appmenuData.length; i++)
		{
			...
			{   
				cnt++;
				var closure = "";//appmenuData[i][3+IndexJL];
        ...
        if(moreExists)
        {
			...
			//Adding popup layer appmenu elements for appmenus count>4
        ...
        eventHandler: function(eventObject, target, sourceFormID){
            ...
            if(appmenuId == 'appmenumore'){
                //closure = this.showmoreappmenuitems();
                closure = $KW.Appmenu.showmoreappmenuitems;
            }else{
		...
		if(appmenuWidgetModel.focusskin)
		{
			if($KW.Appmenu.focusIndex != "")    
			{
				...
				{
					//Adding default skin to all appmenus
			...
			$KW.Appmenu.focusIndex = index;
			//setting focus skin to appmenu clicked
			if(! (targetNode.id == "appmenumore") )
			{
    ...
    setappmenufocusindex: function (index){
        ...
        if(appmenuWidgetModel.focusskin)
        {
			...
			var labelEle = document.querySelectorAll('#'+targetNode.id +'  label');
			if(labelEle) labelEle[0].className = appmenuWidgetModel.focusskin;
			$KW.Appmenu.focusIndex = index-IndexJL;
        }
    },
    ...
    hideappmenuitems: function(menulist){
        ...
        // case where appmenu is not rendered yet
		appmenuNode && (appmenuNode.innerHTML=$KW.Appmenu.render("__appmenu",widgetGenerationContext));

    },

    showappmenuitems: function(menulist){
         
        /** Venkat Commented on 10/11/2011 as we have changed the model for appmenu */
        //var appmenuNode = $KU.getElementById("konyappmenudiv");      
        var appmenuWidgetModel = $KG["__appmenu"]; //kony.model.getWidgetModel("__appmenu");
        var tempmodel = appmenuWidgetModel.data;      
        ...
        if(appmenuNode)	
        {	
            appmenuNode.innerHTML = $KW.Appmenu.render("__appmenu",widgetGenerationContext);		
        } 
        else
        {

			/*var main = $KU.getElementById("__MainContainer");
			// appmenu not rendered yet
			if(!main)
				return; */
            ...
            temp.innerHTML = $KW.Appmenu.render("__appmenu",widgetGenerationContext);
    ...
    {        
        var appmenuWidgetModel = $KG["__appmenu"];//kony.model.getWidgetModel("__appmenu");        
        var appmenuData = appmenuWidgetModel.data; 
        var itemExists=false; //added for checking the same appmenuid
    ...
    createappmenu: function(appmenuid, appmenu, skinid, onfocusSkinid) {
       /* if(!skinid || !onfocusSkinid)
           throw new KonyError(1900, "Error", "No Skin exists with the given skin id"); */
    ...
    setcurrentappmenu: function(appmenuid) {
        ...
        var appmenuWidgetModel = $KG["__appmenu"];//kony.model.getWidgetModel("__appmenu");
        var appmenuData = appmenuWidgetModel.data;        
        // Added for Dynamically Setting the appmenu
        var appmenuNode = $KU.getElementById("konyappmenudiv");    
        var widgetGenerationContext = new $KW.WidgetGenerationContext();
        widgetGenerationContext.setdynamically = true;
        if(appmenuNode)	
    ...
    setappmenufocusbyid:function(appMenuItemId) {
        ...
        if(itemExists)
            $KW.Appmenu.setappmenufocusindex(i);  
    },

    addappmenuitemat:function(appmenuid, index, appmenuitem) {
        $KW.Appmenu.addappmenuitem(appmenuitem, index);
		$KW.Appmenu.focusIndex = "";
    },

    removeappmenuitemat:function(appmenuid, index) {
        var appmenuData=$KG["__appmenu"].data;
        $KW.Appmenu.removeappmenuitem(appmenuData[index][IndexJL]);
		if($KW.Appmenu.focusIndex == index)
			$KW.Appmenu.focusIndex = "";
    }
}