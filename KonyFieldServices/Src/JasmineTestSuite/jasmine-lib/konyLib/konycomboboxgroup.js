            ...
            case "masterdata":
                ...
                if (element) {
					...
					*/
					...
					temp.innerHTML = this.generateList(widgetModel, data, {tabpaneID: element.getAttribute("ktabpaneid")});
					element.parentNode.replaceChild(temp.firstChild, element);
					//element.parentNode.innerHTML = this.generateList(widgetModel, data, {tabpaneID: element.getAttribute("ktabpaneid")});
    ...
    generateList: function(comboboxModel, data, context){
		...
		if(!comboboxModel.view  || comboboxModel.view == 'native'){
			if (data.length > IndexJL) {			
				...
				comboboxModel.selectedkey = key ? key : data[IndexJL][IndexJL]; //First item will be selected if there is no selectedkey
		...
		htmlString = "<select "+ (kony.appinit.isIE7 || kony.appinit.isIE8 ? " onchange='$KW.ComboBox.eventHandler(arguments[0],this)'": " ") + $KW.Utils.getBaseHtml(comboboxModel, context) + "class='" + computedSkin + "'" + (comboboxModel.disabled ? " disabled='true'" : "") + " style='"+$KW.skins.getBaseStyle(comboboxModel, context)+ "'>";			
        ...
        if (comboboxModel) {
			...
			if(target.getAttribute("kcontainerID"))
				...
				if ((comboboxModel.ondone || comboboxModel.onselection) && comboboxModel.blockeduiskin) {
					$KW.Utils.applyBlockUISkin(comboboxModel);
					
				}
                var comboBoxHandlr = $KU.returnEventReference(comboboxModel.ondone || comboboxModel.onselection);
				if(target.tagName == 'DIV' && picker && prevSelectedKey !== picker.val) {
					comboBoxHandlr && comboBoxHandlr.call(comboboxModel,comboboxModel);
