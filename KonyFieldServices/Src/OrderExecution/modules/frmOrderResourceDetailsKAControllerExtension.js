...
kony.sdk.mvvm.frmOrderResourceDetailsKAControllerExtension = Class(kony.sdk.mvvm.CustomFormControllerExtensionMF,{
	...
	fetchMasterData: function(serviceName, options, successcallback, errorcallback) {
        try {
			...
			scopeObj.$class.$superp.fetchMasterData.call(scopeObj, serviceName, options, successcallback, errorcallback);
        } catch (e) {
    ...
    bindData: function(dataMap) {
        try {
    		...
    		processedData["lblQuantityNeededValueKA"] =formData["RequestedQuantity"]?String(utilitiesObj.roundNumber(formData["RequestedQuantity"],2)):"";
    ...
    navigateBack: function(doReload) {
		try {
            ...
            kony.sdk.mvvm.log.error("Error in Blogic navigateBack : " + error);
        }
    },
    deviceBack : function() {    
