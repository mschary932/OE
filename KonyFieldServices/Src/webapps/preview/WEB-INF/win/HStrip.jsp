...
<input type="hidden" id="<%=widgetid%>Skin"/>       <input type="hidden" id="<%=widgetid%>FocusSkin"/>
<%
...
<td>
    ...
    class="previousArrowNav <% if(pageNo == 0){ %>inactive<%}%>"  id="<%=widgetid%>" konywidgettype = "Khstrip"    eventname="<%=widgetid%>.prev.event_.Khstrip">
    </a>
</td>
     ...
     <%
         for(int i=startRecord; i >= 0 && i < endRecord && i < size; i++)
        {
            ...
            <img class="<%=imgSkin%>"   src = "<%=imghimgstrip%>"        eventname="<%=widgetid%>.<%=i%>.event_.Khstrip"
                 konywidgettype = "Khstrip" id="<%=widgetid%>" <%=widget.map.get(WEBConstants.ENABLED)%>
                <% if(imageCat != null && imageCat.trim().length() != 0) {%>
                  style="padding: <%=JSWAPUtil.getWidgetPadding(widget)%>; margin-left:<%=image_left_margin%>px; margin-right:<%=imageSpace%>px"
                <%}%>                 prejsevent="<%=widget.getTable(constants.PRE_ONCLICK_JS)%>" postjsevent="<%=widget.getTable(constants.POST_ONCLICK_JS)%>" />
            <%if(event.equals("yes")){%>
                  </a>
            <%}%>
...
<td>
           <a href="<% if(endRecord >= totalRecords){ %>#<%}else{%><%=response.encodeURL(apppath+"?formid=" +formid+"&cat=bjs"+ (WAPUtilities.getKRFId(request) != null && WAPUtilities.getKRFId(request).length() >0 ? "&krfid="+WAPUtilities.getKRFId(request) : "" ) +"&node="+request.getAttribute("node.no")+"&"+widgetid+".next.event_.Khstrip=x")%><%}%>"
                class="nextArrowNav <% if(endRecord >= totalRecords){ %>inactive<%}%>"
