...
<div id="<%=widgetid%>" konywidgettype="KTouchhstrip" eventname="<%=widgetid%>.event_.Khstrip"
    ...
    <table style="table-layout: auto;width:100%;border-collapse:collapse">
        <tr>
			<% if(pageNo != 0){ %>
            <td>
                <a href="<% if (pageNo == 0) {%>#<%} else {%><%=response.encodeURL(apppath + "?formid=" + formid + "&cat=bjs" + (WAPUtilities.getKRFId(request) != null && WAPUtilities.getKRFId(request).length() > 0 ? "&krfid=" + WAPUtilities.getKRFId(request) : "") + "&node=" + request.getAttribute("node.no") + "&" + widgetid + ".prev.event_.Khstrip=x")%><%}%>"
                   class=""  id="<%=widgetid%>" konywidgettype = "Khstrip"    eventname="<%=widgetid%>.prev.event_.Khstrip">
                    <img src="<%=imgpath+"leftarr.png"%>" /> 
                </a>
            </td>
			<%}%>
            <td>
                <div id="<%=widgetid%>" style="width:100%;font-size:0px;">
                    <%
                  ...
                  <% boolean isWidEnabled = ("".equals(widget.map.get(WEBConstants.ENABLED)))?true:false ;%>
                    <%if (event.equals("yes") && isWidEnabled) {%>
                    <a href="<%=response.encodeURL(apppath + "?formid=" + formid + "&cat=bjs" + (WAPUtilities.getKRFId(request) != null && WAPUtilities.getKRFId(request).length() > 0 ? "&krfid=" + WAPUtilities.getKRFId(request) : "") + "&node=" + request.getAttribute("node.no") + "&" + widgetid + "." + i + ".event_.Khstrip=x")%>">
                        ...
                        <img class="<%=imgSkin%>"   src = "<%=imghimgstrip%>"    <%if (isWidEnabled) {%>     eventname="<%=widgetid%>.<%=i%>.event_.Khstrip" <% }%>
                             view ="fittodimensions"  refwidth = "<%=referenceWidth%>" refheight = "<%=referenceHeight%>" 
                             konywidgettype = "Khstrip" id="<%=widgetid%>"  onload ="galleryLoadHandler2(this)" 
                             <% if (imageCat != null && imageCat.trim().length() != 0) {%>
                             style="padding: <%=JSWAPUtil.getWidgetPadding(widget)%>; margin-right:<%=imageSpace%>px
            ...
            <% if(endRecord < totalRecords){ %>
            <td>
                <a href="<% if (endRecord >= totalRecords) {%>#<%} else {%><%=response.encodeURL(apppath + "?formid=" + formid + "&cat=bjs" + (WAPUtilities.getKRFId(request) != null && WAPUtilities.getKRFId(request).length() > 0 ? "&krfid=" + WAPUtilities.getKRFId(request) : "") + "&node=" + request.getAttribute("node.no") + "&" + widgetid + ".next.event_.Khstrip=x")%><%}%>"
                   class=""
                   id="<%=widgetid%>" konywidgettype = "Khstrip" eventname="<%=widgetid%>.next.event_.Khstrip">
                    <img src="<%=imgpath+"rightarr.png"%>" />
                </a>
            </td>
            <%}%>
        </tr>
