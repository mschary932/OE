...
<table columns=<%=columns%> class="<%=widgetSkin%>" style="<%=wSkin%>;<%="true".equals(bcollapse) ? "border-collapse:collapse;" : ""%>"
            ...
            for (String s : childWidgets)
            {
                    ...
                    if (childVisible)
                    {
                        ...
                        else
                        {
                            ...
                            if (!(jspFile == "Phone.jsp") && !(childWidget instanceof KonyLabel) && !(childWidget instanceof KonyRichText)
                            ...
                            request.setAttribute(WEBConstants.WIDGET_ID, childWidget.getWidgetID());
                            String adjustedCompWeight = JSWAPUtil.getDepthBasedWidgetContainerWeightForIE(childWidget, columns);
                            request.setAttribute("adjustedCompWeight", adjustedCompWeight);
                        %>
                        <td style="width:<%=childwidth%>; padding:<%=childWidgetMargin%>; border:none;" <%=JSWAPUtil.getWidgetAlignmentStyle(childWidget)%> >
                            ...
                            <a konywidgettype = "Kbox" eventname="<%=eventName%>event_" style="text-decoration:none;"
                                    ...
                                    WAPUtilities.getKRFId(request).length() >0 ? "&krfid="+WAPUtilities.getKRFId(request) : "" ) +"&event="+containereventName +"&previousform=" + 
