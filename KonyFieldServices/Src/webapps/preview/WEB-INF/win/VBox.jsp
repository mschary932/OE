...
<%
	...
	List<String> childWidgets = ((LuaTable) widget.getTable(WEBConstants.CHILDREN)).list;
        ...
        styleMap.put("width", JSWAPUtil.getDepthBasedWidgetContainerWeightForIE(widget, 1));
		...
		for (String s : childWidgets)
		{
			...
			if (childVisible)
			{
                                    ...
                                    && !(childWidget.map.get("wType").equals("Link")) && childWidget.map.get(KonyServerWidget.MARGIN) != LuaNil.nil)
                            {
                                    ...
                                    width = JSWAPUtil.getDepthBasedWidgetContainerWeightForIE(childWidget, 1);
                            }
                            request.setAttribute(WEBConstants.WIDGET_ID, childWidget.getWidgetID());
                            String adjustedCompWeight = JSWAPUtil.getDepthBasedWidgetContainerWeightForIE(childWidget, 1);
                            request.setAttribute("adjustedCompWeight", adjustedCompWeight);
                            %>
                            <div style="margin: <%=childWidgetMargin%>; width: <%=width%>;   border: none;" <%=widgetAlignment%> >
                              <%if( (eventExist) &&(jspFile.equalsIgnoreCase("Label.jsp") ||
                                ...
                                <%if(preOnclickJS != null){ %>prejsevent="<%=preOnclickJS%>" <%}%>
                                        ...
                                        WAPUtilities.getKRFId(request).length() >0 ? "&krfid="+WAPUtilities.getKRFId(request) : "" ) +"&event="+containereventName +"&previousform=" + (uiState.getSessionLevelVariable(WEBConstants.PREVIOUS_FORM_ID) != null ? WAPUtilities.escapeHtml((String)uiState.getSessionLevelVariable(WEBConstants.PREVIOUS_FORM_ID)) : uiState.getSessionLevelVariable(WEBConstants.PREVIOUS_FORM_ID)) )%>"
                                >
                                <%}%>
                            ...
                            <%
                             request.removeAttribute("adjustedCompWeight");
                        }
