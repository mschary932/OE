...
<%
    ...
    if(widget.map.get(constants.PRE_ONCLCICK_JS) != null)
    	request.setAttribute("preOnclickJS",widget.map.get(constants.PRE_ONCLCICK_JS));
    ...
    String margin = JSWAPUtil.getWidgetMargin(widget);
    String percentWidtharray[] = margin.split("%");
    
    String margintop=percentWidtharray[0]+ "% "+percentWidtharray[1] +"% 0% " + percentWidtharray[3] + "%" ;
    String marginbottom="0% "+percentWidtharray[1]+"% " +percentWidtharray[2]+"%  " + percentWidtharray[3] +"%";
    String margininbetween="0% "+percentWidtharray[1] +"% 0% " + percentWidtharray[3] + "%";
    ...
    if (isVisible)
    {
        ...
        if (obj != LuaNil.nil)
        {
            ...
            if (obj != LuaNil.nil && obj instanceof java.util.Hashtable)
            {
                ...
                if (widget.getTable(constants.A_SKIN) != null && widget.getTable(constants.A_SKIN) != LuaNil.nil) {
                    altSkin = (String)widget.getTable(constants.A_SKIN);
                }
                if (widget.getTable(constants.FOCUSSKIN) != null && widget.getTable(constants.FOCUSSKIN) != LuaNil.nil) {
                ...
                if (widget.getTable(constants.FOCUSED_INDEX) != LuaNil.nil && widget.getTable(constants.FOCUSED_INDEX) != null) {
                	focusedIndex = (int) (Double.valueOf(widget.getTable(constants.FOCUSED_INDEX).toString()).intValue());
                }
                ...
                if(KonySegmentUI.VIEW_TYPE_PAGE.equals(widget.map.get(constants.VIEW_TYPE)) || "pageview".equals(widget.map.get("view")))
                {
                 ...
                 %>
                          ...
                          <% if(focusedIndex == 0){focusedIndex = focusedIndex+(int)constants.INDEXJL;}
                 				i = focusedIndex-(int)constants.INDEXJL;
                                request.setAttribute("dfltSelection", widget.map.get(constants.DFLT_SEL));
	                        ...
	                        <tr align="left" style = "width:100%;">
	                           <td align="left" style="width:30px;">
	                            ...
	                            <td align="center" style="width:40px;"> 
	                                 <span><%=i+1%> of <%=segmentdata.size()%> </span>
	                            </td>  
                ...
                else if(widget.getTable("sections") != LuaNil.nil)
                {
                    ...
                    java.util.List sectionList = (java.util.List) obj;
                    ...
                    for(int ss=0; ss<sectionList.size(); ss++)
                    {
                        ...
                        if(sectionHeaderTemplate != null)
                        {
                                 ...
                                 //request.setAttribute("childrenarray", headerTemplateChilren.list.toArray(chilren));
