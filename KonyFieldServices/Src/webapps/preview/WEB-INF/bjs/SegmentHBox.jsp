...
<%
	//KonyServerWidget widget = (KonyServerWidget) form.map.get(widgetId);
        ...
        request.setAttribute("segmenthbox", "true"); String bcollapse = "";
		if(widget.getTable(constants.BRDR_COLLAPSE) != null && widget.getTable(constants.BRDR_COLLAPSE) != LuaNil.nil)
			bcollapse = (String)widget.getTable(constants.BRDR_COLLAPSE);
         
        if(request.getAttribute("eventName") != null && request.getAttribute("eventName") != LuaNil.nil)
            eventName = (String) request.getAttribute("eventName");
        if(request.getAttribute("eventExist") != null && request.getAttribute("eventExist") != LuaNil.nil)
