...
%>
<div id="<%=eventName%>" name="<%=eventName%>" <%if (widget.map.get(constants.ONCLICK) != null)
                {%>
	...
	<%if (widget.map.get(constants.PRE_ONCLICK_JS) != null
                ...
                {%>
	prejsevent="<%=widget.map.get(constants.PRE_ONCLICK_JS)%>" <%}%> rowid="<%=request.getAttribute("segmentrow")%>"
	segmentid="<%=request.getAttribute("segmentid")%>" <%if (request.getAttribute("sectionId") != null)
                {%>
	sectionid="<%=request.getAttribute("sectionId")%>" <%}%> style="<%=style%>" <%if (event.equals("yes"))
                {%>
	event="<%=event%>" <%}%> 
...
<%
    ...
    else if ("nonpercent".equals(request.getAttribute("layout")))
    {
%>
