        ...
        if (obj != null)
        {
           ...
           int size = data.size();
    		...
    		int totalRecords = size;
               ...
               if(widget.map.get("bjs"+imageCat+"heightwidth")!= null ){
    		...
    		if(obj != null && size > 0)
        		...
        		if (obj != null && obj instanceof com.konylabs.vm.LuaTable)
        		{
                  ...
                  %> <table cols = "<%=recPerRow%>" width="100%" style=" padding: <%=JSWAPUtil.getWidgetStyleInfo(widget, true)%>">
                  <%
		...
		%>		
                        <td   valign="middle" align="center">
                            ...
                            WAPUtilities.getKRFId(request).length() >0 ? "&krfid="+WAPUtilities.getKRFId(request) : "" ) +"&node="+request.getAttribute("node.no")+"&rowid="+i+"&event=" + widgetRefStr )%>">
                             <%}
                               if(widget.map.get("bjs"+imageCat+"heightwidth")!= null && !widget.map.get("bjs"+imageCat+"heightwidth").equals("0,0")){
                             %>
                                <img class="<%=imgSkin%>" src="<%=img%> " konywidgettype = "Kimggal"  <%=widget.map.get(WEBConstants.ENABLED)%>
