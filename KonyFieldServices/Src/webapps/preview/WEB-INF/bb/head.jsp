...
<head>    
    <%
          ...
          if(!isPreview) {
              if(appProperties != null)
              {
                  ...
                  hasMap = JSWAPUtil.getBooleanValue(appProperties.map.get(constants.HAS_MAP));                  
   ...
   <%if(deviceCategory.equalsIgnoreCase("nth")){
        ...
        {
        %>
        	<style id="theme" type="text/css" media="screen">@import "<%= String.format("%s/%s%s/kony%sadvie10%s.css?ver=%s", gpath, deviceCategory, WEBUtil.getTheme(), deviceCategory, imageCat,version) %>";</style>
        <%
        }
    ...
    %>
        ...
        //var srcfile = document.getElementById("konycssfile").value;         
   ...
   // kony.system.cookie.createCookie("konydpi", dpi);   
     ...
     <%}%>
    <script type="text/javascript" src="<%=String.format("%s/%s/konycore.js?ver=%s",gpath, deviceCategory,version)%>"></script>
    <script type="text/javascript" src="<%=String.format("%s/%s/konysystem.js?ver=%s",gpath, deviceCategory,version)%>"></script>
    <script type="text/javascript" src="<%=String.format("%s/%s/konywidgets.js?ver=%s",gpath, deviceCategory,version)%>"></script>
    <script type="text/javascript" src="<%=String.format("%s/%s/konytouchwidgets.js?ver=%s",gpath, deviceCategory,version)%>"></script>    
    <script type="text/javascript" src="<%=String.format("%s/%s/konycalendar.js?ver=%s",gpath, deviceCategory,version)%>"></script>
    
    <script type="text/javascript" src="<%=gpath%>/tparty/cal/yahoo-dom-event.js"></script>
    <script type="text/javascript" src="<%=gpath%>/tparty/cal/calendar-min.js"></script>
    <script type="text/javascript" src="<%=gpath%>/tparty/cal/element-min.js"></script>
    <script type="text/javascript" src="<%=gpath%>/tparty/cal/container-min.js"></script>
    ...
    <%if(true) { %>
        <script type="text/javascript" src="<%=gpath%>/<%=deviceCategory%>/konygps.js?ver=<%=version%>"></script>
        <script type="text/javascript" src="<%=gpath%>/<%=deviceCategory%>/konymaps.js?ver=<%=version%>"></script>        
    <% } %>
    <script type="text/javascript" src="<%=gpath%>/<%=deviceCategory%>/popup.js?ver=<%=version%>"></script>
    <script type="text/javascript" src="<%=gpath%>/<%=deviceCategory%>/autocomplete.js?ver=<%=version%>"></script> 
    <style type="text/css" media="screen">@import "<%=gpath%>/<%=deviceCategory%>/autocomplete.css?ver=<%=version%>";</style>
    ...
    %>
	    <script type="text/javascript" src="<%=upath%>/<%=deviceCategory%>/tchybrid.js?ver=<%=version%>"></script>
	    <script type="text/javascript" src="<%=upath%>/<%=deviceCategory%>/nativeplatform.js?ver=<%=version%>"></script>
    <%
        }%>
