...
<%
        ...
        String widgetSkin = "";
        if(widget.getTable(KonyServerWidget.SKIN) != com.konylabs.vm.LuaNil.nil) {
           ...
           for(int i=0; i<choiceKeys.size(); i++)
            {
                ...
                %>
                <%if ("vertical".equals(orientation)){%>
                   <div style="<%=wSkin%> height: auto;" class="<%=widgetSkin%>">
                <%}%>                
                <input  style="text-align:left; vertical-align:middle;" type="checkbox" <%=selected%> value="<%=i%>" <%=widget.map.get(KonyServerWidget.ENABLED)%>
                        name ="<%=name%>" id ="<%=name%>"
                        konywidgettype = "Kcheckboxgroup"
                <%if(widget.map.get(constants.PRE_ONCLICK_JS) != null && widget.map.get(constants.PRE_ONCLICK_JS) != com.konylabs.vm.LuaNil.nil){ %>prejsevent="<%=widget.map.get(constants.PRE_ONCLICK_JS)%>"
                <%} if(widget.map.get(constants.POST_ONCLICK_JS) != null && widget.map.get(constants.POST_ONCLICK_JS) != com.konylabs.vm.LuaNil.nil){ %> postjsevent="<%=widget.map.get(constants.POST_ONCLICK_JS)%>" <%}%>

                        <% if(widget.map.get(constants.ONSELECTION) != null) {%>
                            event = "yes"
                        <% } else { %>
                            event = ""
                        <% } %>
