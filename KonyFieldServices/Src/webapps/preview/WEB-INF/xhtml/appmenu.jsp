...
<div class="<%=appskinId%>">
		...
		<tr>
			...
			<td style="width:<%=wid%>;" valign="middle" align="center"><a
				...
				<%if(WAPUtilities.isSecureTransaction(session))
					{%>
				href="<%=response.encodeURL(apppath + "?&formid=" + frmId + "&krfid=" + WAPUtilities.getKRFId(request) + "&previousform="
								+ uiState.getSessionLevelVariable(WEBConstants.PREVIOUS_FORM_ID) + "&" + appMenuid + "." + count
								+ ".event_.Kappmenu=x")%>"
				style="text-decoration: none;"> <%
 	}
 			else
 			{
 %> href="<%=response.encodeURL(apppath + "?&formid=" + frmId + "&previousform="
								+ uiState.getSessionLevelVariable(WEBConstants.PREVIOUS_FORM_ID) + "&" + appMenuid + "." + count
								+ ".event_.Kappmenu=x")%>"
					style = "text-decoration: none;"> <%
 	}
 ...
 %>
			<td style="<%=wid%>;" valign="middle" align="center">
				...
				<table>
					<tr>
						<td style="text-align: center"><a konywidgettype="Kappmenu"
							<%if(skinId != null && skinId.trim().length() != 0)%>
							class="<%=skinId%>"
							<%if(WAPUtilities.isSecureTransaction(session))
					{%>
							href="<%=response.encodeURL(apppath + "?appmenumore=true&formid=" + frmId + "&krfid=" + WAPUtilities.getKRFId(request)
								+ "&previousform=" + uiState.getSessionLevelVariable(WEBConstants.PREVIOUS_FORM_ID) + "&" + appMenuList.get(4).id
								+ "." + 4 + ".event_.Kappmenu=x")%>"
							style="text-decoration: none;"> <%
 	}
 			else
 			{
 %> href="<%=response.encodeURL(apppath + "?appmenumore=true&formid=" + frmId + "&previousform="
								+ uiState.getSessionLevelVariable(WEBConstants.PREVIOUS_FORM_ID) + "&" + appMenuList.get(4).id + "." + 4
								+ ".event_.Kappmenu=x")%>"
								style = "text-decoration: none;"> <%
 	}
 %> <img
