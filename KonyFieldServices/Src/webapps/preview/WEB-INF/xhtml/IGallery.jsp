    ...
    <%  
        ...
        if (obj != null)
        {
           ...
           int size = data.size();
    		...
    		int totalRecords = size;
               ...
               if(widget.map.get("basic"+imageCat+"heightwidth")!= null ){
                    String heightwidth = (String)widget.map.get("basic"+imageCat+"heightwidth");
                    dim = heightwidth.split(",");
                }
    		...
    		if(obj != null && size > 0)
        		...
        		if (obj != null && obj instanceof com.konylabs.vm.LuaTable)
        		{
                  ...
                  %> <table cols = "<%=recPerRow%>" style="width:100%; padding:0px;">
                  <%
		...
		%>		
                        <td  valign="middle" align="center">
                            <% if(event.equals("yes")) {%>
                            <a href="<%=response.encodeURL(apppath+"?formid=" + formid + "&cat=basic"+ (WAPUtilities.getKRFId(request) != null &&
                            WAPUtilities.getKRFId(request).length() >0 ? "&krfid="+WAPUtilities.getKRFId(request) : "" ) +"&node="+request.getAttribute("node.no")+"&rowid="+i+"&event=" + widgetRefStr )%>">
                             <%}
                               if(widget.map.get("basic"+imageCat+"heightwidth")!= null && !widget.map.get("basic"+imageCat+"heightwidth").equals("0,0")){
                             %>
                                <img class="<%=imgSkin%>"  src="<%=img%>" konywidgettype = "Kimggal"
                                   style = "margin-top:<%=image_top_space%>px;margin-right:<%=imageSpace%>px;margin-bottom:<%=imageSpace%>px;
                                   margin-left:<%=image_left_space%>px; width:<%=dim[0]%>px; height:<%=dim[1]%>px;"                                   
                                 />
                              <% }else{ %>
                                <img class="<%=imgSkin%>"  src="<%=img%>" konywidgettype = "Kimggal"
                                   style = "margin-top:<%=image_top_space%>px;margin-right:<%=imageSpace%>px;margin-bottom:<%=imageSpace%>px;
                                   margin-left:<%=image_left_space%>px;"                                   
                                 />
                              <% } %>
                             <% if(event.equals("yes")){%>
                              </a>
        	...
        	<% if(startRecord > 1){  %>
        		...
        		href="<%=response.encodeURL(apppath+"?formid=" + formid + "&cat=basic"+ (WAPUtilities.getKRFId(request) != null &&
        	...
        	<% } %>
        	 ...
        	 <td colspan="<%=(recPerRow-2)%>" align="center"> 
