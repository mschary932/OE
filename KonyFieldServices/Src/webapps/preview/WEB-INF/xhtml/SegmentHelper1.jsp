                    ...
                    if (obj != null && obj != com.konylabs.vm.LuaNil.nil)
                    {
                       ...
                       if("true".equals(widget.getTable(constants.SEPERATOR_REQD).toString()) && widget.getTable(constants.SEPCOLOR)!= null){
                        ...
                        if (wapUtil.areSegmentWidgetsVisible(segmentmetadata, segmentrec, segmentchildrenlist.toArray(segmentchildrenarray)))
                        {
                            ...
                            {%>
                                <table class = "<%=segskin%>"
                                    <%if("horizontal".equals(segmentorientation)){%>  columns = "<%=segmentchildrenlist.size()%>"
                                        <%} else {%> columns = "1"
                                    <%}%>
                                    <% if (i == 0) {%>
                                    style = "<%=wSkin%> border-bottom : <%=sepLine%>;  border-top:<%=sepLine%>;"
                                    <%} else if (i != segmentdata.size() - 1) {%>
                                    style = "border-bottom:<%=sepLine%>; <%=wSkin%>"
                                    <%} else {%>
                                    style = "<%=sepLine%>;  <%=wSkin%>"
                                    <%}%>
                                >
                            <%} else { %>
                               <table
                                ...
                                columns = "<%=segmentchildrenlist.size()%>"
                                <%} else {%>
                                columns = "1"
                                <%}%>
                                class = "<%=segskin%>"
                            ...
                            for (String s : segmentboxchildren)
                            {
                                ...
                                String cwt = null;
                                if (childWidget1.getTable(constants.CONTAINER_WEIGHT) != LuaNil.nil)
				{
                                        int temp = (int)(Double.parseDouble(childWidget1.getTable(constants.CONTAINER_WEIGHT).toString()));
                                ...
                                {%>
                                    <%if(childVisible) {%>
                                        <td style = "width:<%=cwt%>; border:none;  text-align:<%=childWidget1.map.get(constants.CONTENT_ALIGNMENT)%>;"
