...
kony.servicesapp.LOGIN_BATCH_SIZE = 500000;
kony.servicesapp.APP_TYPE = {
    "APPTYPE": "MFAPP"
};
kony.servicesapp.CONNECTOR="ECC";
...
kony.servicesapp.versionNumber = "3.1.0.4";
...
kony.servicesapp.ENTITY_WORKORDERMATERIAL = "WorkOrderMaterial";
...
kony.servicesapp.ENTITY_EAMWOOPERATION = "EAM_WO_OPERATION";
...
kony.servicesapp.MAXIMUMAGE_GPS=360000;
...
kony.servicesapp.FRMORDERSVIEWSKA="frmOrdersViewsKA";
kony.servicesapp.FRMORDERCOMPLETECHECKLISTKA="frmOrderCompleteCheckListKA";
...
kony.servicesapp.ANIMATION_TBXPASSWORD_TOP = "183dp";
...
kony.servicesapp.DEFAULT_SURVEY_CONFIG = {
    ...
    "flxSurveyKA": {
        "fieldprops": {
            ...
            "widgettype": "Label",
            "query": "select res.* from (select que.* from (select foi.FieldOptionGroup_id, foi.id, foi.code ,foi.label as optionitem_name, sfd.isrequired ,sfd.DataType_id as datatype_id, sfd.name, sfd.surveydefinition_id, sd.name as surveyname ,sfd.label, sfd.id as surveyfielddefinition_id, ss.name as surveysectionname, ss.id as surveysection_id from SurveyFieldDefinition as sfd left join FieldOptionItem as foi on foi.FieldOptionGroup_id = sfd.FieldOptionGroup_id left join SurveySectionField as ssf on ssf.FieldDefinition_id=sfd.id and ssf.SurveyDefinition_id= sfd.SurveyDefinition_id left join SurveySection as ss on ss.id=ssf.SurveySection_id  left join SurveyDefinition as sd on sd.id = sfd.SurveyDefinition_id where sfd.SurveyDefinition_id = (select wos.SurveyDefinition_id from EventSurveyDefinition wos where wos.EventType_id = '{x}')) as que order by length(surveyfielddefinition_id)) as res",
            "querytype": "sql",
            "field": "Description"
        }
    },
...
kony.servicesapp.FORMS_SYNC_PROGRESS_BAR = {
	...
	"frmCompleteOrderAcceptanceKA" : {"processBar" : false, "refresh" : false},
	...
	"frmDirectionsKA" : {"processBar" : true, "refresh" : true},
	...
	"frmOrderCompleteCheckListKA" : {"processBar" : false, "refresh" : false},
